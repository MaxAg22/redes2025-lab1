...
def test_get_movie_by_genre():
    response = requests.get(
        'http://localhost:5000/peliculas/get_movie_by_genre?genre=Acción'
        )
    assert response.status_code == 200


def test_get_movie_by_title_keyword():
    response = requests.get(
        'http://localhost:5000/peliculas/get_movie_by_title_keyword?kw=Th'
        )
    assert response.status_code == 200
...
def test_random_movie_by_genre():
    response = requests.get(
        'http://localhost:5000/peliculas/random_by_genre?genre=Drama'
        )
    assert response.status_code == 200


def test_film_for_holiday():
